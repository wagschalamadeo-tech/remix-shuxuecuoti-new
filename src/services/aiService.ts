            ...
            parts: [
              {
                ...
                4. 如果有标准答案，提取出来。
                5. 判断该题目的核心知识点。
                   - **重要：必须使用小学阶段通俗易懂的术语，并注明解题方法**（例如：使用“鸡兔同笼（假设法）”而不是“二元一次方程”，使用“和倍问题（画图法）”而不是“代数方程”）。
                请以JSON格式返回。`,
              },
              {
                inlineData: {
        ...
        contents: [
          {
            parts: [
              {
                text: `基于以下原题 and 知识点，生成3道“举一反三”的变式题。
                原题：${originalQuestion}
                知识点：${knowledgePoint}
                ...
                3. 每道题必须包含：
                   ...
                   - explanation: 小助手讲解（即思路引导）。要求：
                     ...
                     - **禁止使用 <br> 标签。**
                     - **强制换行格式：每一句话（以句号、问号、感叹号结尾）之后必须紧跟一个 \n 换行符。绝对禁止两句话出现在同一行。**
                   - steps: 具体的解题步骤。
                     - 在每一步骤之前，必须先解释“为什么要这么做”（即解题逻辑和原因），然后再给出具体的计算或操作。
                     - **强制换行格式：步骤中的每一句话也必须独立成行（使用 \n 换行）。**
                   - answer: 最终正确答案。
                   - difficulty: 难度等级。
                请以JSON格式返回。`,
              },
